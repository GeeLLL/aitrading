...
TRADABILITY_PROBE_REQUIRED_TOOLS = frozenset({"get_equity_tradability"})


def collect_tradability_probe(
    symbol: str,
    *,
    project_root: str | Path = ".",
    vault_root: str | Path = "logs/raw",
    ...
    """Vault-stored probe proving the instrument session is read live.

    The instrument-session market check previously PASSED on a string blob the
    gate agent typed: the adjudicator validated the claim's shape but nothing
    proved a get_equity_tradability call ever happened. This probe makes that
    call the same way the fresh-quote probe does — one tool, harvested
    byte-faithfully, stored in the immutable vault — so the check rests on
    evidence rather than self-report.
    """

    normalized_symbol = symbol.strip().upper()
    if not SYMBOL_PATTERN.fullmatch(normalized_symbol):
        raise OfficialCollectorError("Invalid equity symbol for tradability probe.")
    root = Path(project_root).resolve()
    prompt = (root / "prompts/robinhood_tradability_probe.md").read_text(encoding="utf-8").format(
        symbol=normalized_symbol
    )
    command = [
        claude_binary(),
        ...
        "--output-format", "stream-json",
        "--verbose",
        "--allowedTools", f"mcp__{MCP_SERVER_NAME}__get_equity_tradability",
        "--disallowedTools", ",".join(EXPLICITLY_DISALLOWED_TOOLS),
    ]
    try:
        completed = subprocess.run(
        ...
        raise OfficialCollectorError("Tradability probe failed or timed out.") from error
    if completed.returncode != 0:
        raise OfficialCollectorError(
            "Tradability probe returned no valid result. "
            f"exit={completed.returncode}; "
            f"{_safe_failure_detail(getattr(completed, 'stderr', None))}"
        )
    expected_prefix = f"mcp__{MCP_SERVER_NAME}__"
    requests, responses, response_texts, _truncated, skipped_lines = _harvest_stream(
        completed.stdout, expected_prefix, resilient=False,
        required_tools=TRADABILITY_PROBE_REQUIRED_TOOLS,
    )
    received_at = datetime.now(timezone.utc)
    request_envelope: dict[str, object] = {
        "schema_version": 1,
